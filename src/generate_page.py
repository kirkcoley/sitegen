...
def generate_page(from_path, template_path, dest_path, basepath='/'):
    ...
    doc = doc.replace('href="/', f'href="{basepath}')
    doc = doc.replace('src="/', f'src="{basepath}')
    ...
    for file in cont:
        ...
        if os.path.isfile(sname):
            if sname.endswith(".md"):
                generate_page(sname, template_path, dname.replace("md", "html"), basepath)
            else:
                continue
        else:
            if not os.path.exists(dname):
                os.mkdir(dname)
            generate_pages_recursive(sname, template_path, dname, basepath)
